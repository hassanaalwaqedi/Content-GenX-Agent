...
def fetch_transcripts_batch(video_ids: List[str]) -> dict[str, str]:
    ...
    Returns a dict mapping video_id -> transcript text.
    Videos without transcripts will have empty strings.
    """
    results = {}
    success_count = 0
    ...
    logger.info(
        "Transcript batch complete: %d/%d videos have transcripts.",
        success_count,
        len(video_ids),
    )
    return results
