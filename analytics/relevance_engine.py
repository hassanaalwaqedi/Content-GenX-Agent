...
class ContentRelevanceEngine:
    ...
    def score_batch(
        ...
        """
        Score a batch of content items.  Returns (passed, rejected) tuple.

        Each passed item gets relevance metadata injected:
            content["relevance_score"]    = 82
            content["matched_keywords"]   = "cute kid,cute"
            content["matched_hashtags"]   = "#cutekid,#cutekids"
            content["match_reason"]       = "Keyword in title · Hashtag match"
        """
        ...
        for content in contents:
            try:
                result = self.score(content, query_context)
                total_score += result.score

                # Inject relevance metadata
                content["relevance_score"] = result.score
                content["matched_keywords"] = ",".join(
                    result.matched_keywords
                ) if result.matched_keywords else ""
                content["matched_hashtags"] = ",".join(
                    result.matched_hashtags
                ) if result.matched_hashtags else ""
                content["match_reason"] = result.match_reason
                ...
                if result.passed:
                    ...
                    logger.debug(
                        "[FILTERED] video_id=%s reason=%r score=%d",
                        content.get("video_id", content.get("id", "?")),
                        result.match_reason,
                        result.score,
                    )
                ...
                content["matched_keywords"] = ""
                content["matched_hashtags"] = ""
                content["match_reason"] = "Scoring error"
                passed.append(content)

        avg_score = total_score / max(len(contents), 1)

        logger.info(
            "[RELEVANCE] scored=%d passed=%d rejected=%d avg_score=%.1f threshold=%d",
            len(contents),
            len(passed),
            len(rejected),
            avg_score,
    ...
    def score_content_item(
        ...
        """
        Score a single item and inject metadata.  Returns the modified dict.

        Does NOT filter — always returns the content with relevance metadata added.
        """
        result = self.score(content, query_context)
        content["relevance_score"] = result.score
        content["matched_keywords"] = ",".join(
